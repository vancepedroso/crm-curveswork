...
import { customersApi, projectsApi } from "./api"  // <── new import
...
const COMPANY = {
  name:"DK Roofing",
  address:"159 New Plymouth, New Zealand",
  email:"info@dkroofing.com",
  phone:"021 555 1234",
  gst:"123-456-789",
  bank:"ANZ 01-2345-6789012-00",
}

// ─────────────────────────── STORAGE (REMOVED – now using API) ──
// (no more window.storage or sGet/sSet)

// ─────────────────────────── HELPERS ───────────────────────────
const uid  = () => Math.random().toString(36).slice(2,10)
const fmt  = n  => "$"+Math.round(n).toLocaleString()
const fmtD = d  => d ? new Date(d+"T12:00:00").toLocaleDateString("en-NZ",{day:"numeric",month:"short",year:"numeric"}) : "—"
...
function calcEst(e) {
  ...
  const flashCost = (e.flashings||0) * 28
  const gutCost   = (e.guttering||0) * 45
...
// ─────────────────────────── SEED DATA (used only as fallback) ──
...
const seed_projects = [
  ...
  { id:"p5",customerId:"c5",address:"5 Trade Lane, Mt Wellington, Auckland", status:"New Lead",   area:0,   roofType:"",                 notes:"Large commercial building. Needs site visit first.",            quoteNum:"",       quoteDate:"",           createdAt:"2024-12-13", estimate:null },
...
// ─────────────────────────── DASHBOARD ───────────────────────────
function Dashboard({ projects, customers, setView, setSelectedProject }) {
  const stats = useMemo(()=>{
    const won   = projects.filter(p=>p.status==="Won")
        ...
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          ...
          <div style={s.card}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              <Btn primary full onClick={()=>setView("new")}>📸 New Roof Job</Btn>
...
function MeasurementTool({ onGeometryChange }) {
  ...
  const geometry = useMemo(()=>{
    ...
    const processedSections = sections.map((sec,idx)=>{
    ...
    return {
      ...
      total_footprint_m2: parseFloat(processedSections.reduce((a,s)=>a+s.footprint_m2,0).toFixed(2)),
      total_surface_m2:   parseFloat(processedSections.reduce((a,s)=>a+s.surface_m2,0).toFixed(2)),
  ...
  useEffect(()=>{ onGeometryChange&&onGeometryChange(geometry) },[geometry])
  ...
  const drawCanvas = useCallback(()=>{
    ...
    if((activeTool==="flashing"||activeTool==="gutter")&&drawPts.length>0){
      const c=activeTool==="flashing"?"#f59e0b":"#06b6d4"
      ctx.strokeStyle=c; ctx.lineWidth=2.5; ctx.setLineDash(activeTool==="flashing"?[8,4]:[4,2])
      ctx.beginPath(); ctx.moveTo(drawPts[0].x,drawPts[0].y)
      drawPts.forEach(p=>ctx.lineTo(p.x,p.y))
      if(hoverPt) ctx.lineTo(hoverPt.x,hoverPt.y)
      ctx.stroke(); ctx.setLineDash([])
    ...
    ptItems.forEach(pi=>{
      if(pi.type==="downpipe"){
        ...
        const c=PEN_COLORS[pi.subtype]||"#8b5cf6"
  ...
  function handleClick(e){
    ...
    if(activeTool==="section"){
      if(drawPts.length>=3){
        ...
        if(d<15){
          setSections(p=>[...p,{id:uid(),name:`Section ${p.length+1}`,pts:drawPts,closed:true,pitch:"1.15",color:SEC_COLORS[p.length%SEC_COLORS.length]}])
          setDrawPts([]); return
        }
      }
      setDrawPts(p=>[...p,pt])
    ...
    else if(activeTool==="downpipe")   { setPtItems(p=>[...p,{id:uid(),type:"downpipe",  x:pt.x,y:pt.y}]) }
    else if(activeTool==="drain")      { setPtItems(p=>[...p,{id:uid(),type:"drain",     x:pt.x,y:pt.y}]) }
    else if(activeTool==="penetration"){ setPtItems(p=>[...p,{id:uid(),type:"penetration",subtype:penSub,x:pt.x,y:pt.y}]) }
    else if(activeTool==="scale"){
      if(!scaleLine?.p1)       setScaleLine({p1:pt,p2:null,knownM})
      else if(!scaleLine?.p2){ setScaleLine(p=>({...p,p2:pt})); setActiveTool("section") }
    }
  }

  function finishLine(){
    if(drawPts.length>=2) setLineItems(p=>[...p,{id:uid(),type:activeTool,pts:drawPts}])
    setDrawPts([])
  }
            ...
            <div style={{marginLeft:"auto",display:"flex",gap:5}}>
              {(activeTool==="flashing"||activeTool==="gutter")&&drawPts.length>=2&&(
                <button onClick={finishLine}
                  style={{padding:"5px 10px",borderRadius:6,border:"1px solid #10b981",background:"#10b981",color:"#fff",fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>
                  Done ✓
                </button>
              )}
              <button onClick={clearAll}
                style={{padding:"5px 10px",borderRadius:6,border:"1px solid rgba(255,255,255,0.15)",background:"transparent",color:"#64748b",fontSize:11,cursor:"pointer",fontFamily:"inherit"}}>
                Clear All
              </button>
            </div>
          </div>
                ...
                <div key={sec.id} style={{marginBottom:10,paddingBottom:10,borderBottom:"1px solid #f1f5f9"}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:6}}>
                    ...
                    <input value={sec.pitch}
                      onChange={e=>setSections(p=>p.map(s=>s.id===sec.id?{...s,pitch:e.target.value}:s))}
...
function EstimateEngine({ initialArea, onEstimateChange }) {
  ...
  useEffect(()=>{ onEstimateChange&&onEstimateChange(result) },[result])
    ...
    <div style={s.grid2}>
        ...
        <div style={{...s.card,marginBottom:14}}>
          ...
          <FG label="Material Type">
            <select style={s.input} onChange={ev=>{
              ...
              setE(prev=>({...prev,materialLabel:m.label,materialRate:m.rate}))
            }} value={e.materialLabel}>
              {MATERIALS.map(m=><option key={m.label} value={m.label}>{m.label} — ${m.rate}/m²</option>)}
            </select>
          </FG>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            <FG label="Flashings (m)"><input style={s.input} type="number" value={e.flashings} onChange={ev=>upd("flashings")(ev.target.value)}/></FG>
            <FG label="Guttering (m)"><input style={s.input} type="number" value={e.guttering} onChange={ev=>upd("guttering")(ev.target.value)}/></FG>
          </div>
        </div>
        <div style={s.card}>
      ...
      <div>
        <div style={{background:"#0f172a",borderRadius:12,padding:20,color:"#fff"}}>
          ...
          {row("Flashings ("+e.flashings+"m)", fmt(result.flashCost))}
          {row("Guttering ("+e.guttering+"m)", fmt(result.gutCost))}
        ...
        <div style={{...s.card,marginTop:14}}>
          ...
          <div style={{fontSize:12,color:"#64748b",lineHeight:2}}>
            ...
            <div>Flashings: <strong style={{color:"#0f172a"}}>{e.flashings}m @ $28/m</strong></div>
            <div>Guttering: <strong style={{color:"#0f172a"}}>{e.guttering}m @ $45/m</strong></div>
...
// ─────────────────────────── QUOTE VIEW ───────────────────────────
function QuoteView({ project, customer }) {
  ...
  const exp = new Date(new Date(qd).getTime()+30*86400000).toISOString().slice(0,10)

  const lineItems = [
    { desc:`${e.materialLabel} roofing — supply & install`, qty:`${e.adjArea.toFixed(1)} m²`, unit:`$${e.materialRate}`, total:e.matCost },
    { desc:"Flashings — ridge/hip/valley",                 qty:`${e.flashings}m`,             unit:"$28",               total:e.flashCost },
    { desc:"Guttering & downpipes",                        qty:`${e.guttering}m`,              unit:"$45",               total:e.gutCost },
  ...
  return (
    <div style={{maxWidth:660,background:"#fff",border:"1px solid #e2e8f0",borderRadius:14,padding:32}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:28,paddingBottom:22,borderBottom:"2px solid #e2e8f0"}}>
        <div>
          <div style={{fontFamily:"'Syne',sans-serif",fontSize:24,fontWeight:800}}>{COMPANY.name}</div>
          <div style={{fontSize:12,color:"#64748b",marginTop:5,lineHeight:1.9}}>
            {COMPANY.address}<br/>{COMPANY.email} · {COMPANY.phone}<br/>GST No: {COMPANY.gst}
          </div>
        </div>
      ...
      <div style={{display:"flex",justifyContent:"flex-end"}}>
        <div style={{minWidth:240}}>
          {[["Subtotal (excl. GST)", fmt(e.sellPrice)],["GST (15%)", fmt(e.gst)]].map(([l,v])=>(
      ...
      <div style={{borderTop:"1px solid #e2e8f0",paddingTop:18,marginTop:18}}>
        <div style={{fontSize:11,color:"#94a3b8",lineHeight:2}}>
          ...
          <strong style={{color:"#64748b"}}>Payment:</strong> Bank transfer to {COMPANY.name} — {COMPANY.bank}<br/>
...
// ─────────────────────────── NEW PROJECT WIZARD ───────────────────────────
function NewProjectWizard({ customers, projects, onSave, onCancel, existingProject }) {
  ...
  async function save() {
    ...
    if(isNewCust && newCust.name) {
      const nc = { id:uid(), ...newCust }
      newCustomers = [...customers, nc]
      cid = nc.id
    }
    const project = {
      ...form,
    ...
    await onSave(project, newCustomers)
    setSaving(false)
  }
  ...
  return (
    <div>
      ...
      {step===1 && <MeasurementTool onAreaChange={a=>{setArea(a)}}/>}
      ...
      <div style={{display:"flex",justifyContent:"space-between",marginTop:24,paddingTop:18,borderTop:"1px solid #e2e8f0"}}>
        ...
        <div style={{display:"flex",gap:10}}>
          {step<STEPS.length-1 && (
            <Btn onClick={()=>setStep(s=>s+1)} style={{opacity:canNext[step]?1:.5,pointerEvents:canNext[step]?"auto":"none"}}>
              Skip →
            </Btn>
          )}
          {step<STEPS.length-1
            ? <Btn primary onClick={()=>canNext[step]&&setStep(s=>s+1)}>Next →</Btn>
        ...
        <div style={{display:"flex",gap:6}}>
          {["All",...STATUSES].map(st=>(
            <button key={st} onClick={()=>setFilterStatus(st)}
              ...
              {st} {st!=="All"&&<span>({projects.filter(p=>p.status===st).length})</span>}
            </button>
          ))}
        </div>
...
// ─────────────────────────── PROJECT DETAIL ───────────────────────────
function ProjectDetail({ project, customers, projects, setProjects, setView, onEdit }) {
...
function Customers({ customers, setCustomers, projects }) {
  ...
  async function save() {
    try {
      if (editCust) {
        const updated = await customersApi.update(editCust.id, form);
        setCustomers(prev => prev.map(c => c.id === editCust.id ? updated : c));
      } else {
        const created = await customersApi.create(form);
        setCustomers(prev => [...prev, created]);
      }
      setEditCust(null); setShowNew(false);
  ...
  return (
    <div>
      ...
      {showNew && (
        <Modal title={editCust?"Edit Customer":"New Customer"} onClose={()=>{ setShowNew(false); setEditCust(null) }}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          ...
          <div style={{display:"flex",justifyContent:"flex-end",gap:10,marginTop:20}}>
            <Btn onClick={()=>{ setShowNew(false); setEditCust(null) }}>Cancel</Btn>
            <Btn primary onClick={save}>{editCust?"Save Changes":"Create Customer"}</Btn>
          </div>
        </Modal>
      )}
    </div>
  ...
  return (
    <div style={{maxWidth:600}}>
      <div style={{...s.card,marginBottom:16}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          <FG label="Company Name"><input style={s.input} defaultValue={COMPANY.name}/></FG>
          <FG label="GST Number"><input style={s.input} defaultValue={COMPANY.gst}/></FG>
          <FG label="Phone"><input style={s.input} defaultValue={COMPANY.phone}/></FG>
          <FG label="Email"><input style={s.input} defaultValue={COMPANY.email}/></FG>
          <FG label="Address" half><input style={s.input} defaultValue={COMPANY.address}/></FG>
          <FG label="Bank Account"><input style={s.input} defaultValue={COMPANY.bank}/></FG>
        </div>
      ...
      <div style={{...s.card,marginBottom:16}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          <FG label="Default Day Rate ($)"><input style={s.input} type="number" defaultValue={850}/></FG>
          <FG label="Default Margin %"><input style={s.input} type="number" defaultValue={20}/></FG>
          <FG label="GST Rate %"><input style={s.input} type="number" defaultValue={15} readOnly style={{...s.input,background:"#f8fafc"}}/></FG>
          <FG label="Default Wastage %"><input style={s.input} type="number" defaultValue={10}/></FG>
        </div>
      </div>
      <div style={s.card}>
        <div style={{fontWeight:700,marginBottom:14}}>Accessory Rates</div>
        {[["Flashings","$28/m"],["Guttering","$45/m"],["Downpipes","$35/each"],["Underlayment","$8/m²"]].map(([l,v])=>(
...
export default function App() {
  ...
  // ── Load data from backend (with fallback to seed) ──
  useEffect(()=>{
    async function loadData() {
      try {
        ...
        console.warn("Backend not available, using seed data...");
  ...
  const PAGE_TITLES = {
    dashboard:"Dashboard", new:"New Project", pipeline:"Pipeline",
    projects:"Projects", project:"Project Detail", customers:"Customers",
    quote_print:"Quote", settings:"Settings",
  }

  // ── Save project (create or update) via API ──
  async function handleSaveProject(project, newCustomers) {
    try {
      ...
      if (newCustomers) {
        const newCust = newCustomers.find(c => !customers.some(ex => ex.id === c.id));
        if (newCust) {
  ...
  const navView = v => { setView(v); if(v!=="project") setSelectedProject(null) }

  if(!loaded) return (
    <div style={{...s.app,alignItems:"center",justifyContent:"center"}}>
      <div style={{textAlign:"center"}}>
        <img src="/aTopRoof.png" alt="aTopRoof" style={{width:220,marginBottom:12,filter:"invert(0)"}}/>
  ...
  return (
    <>
      ...
      <div style={s.app}>
        <Sidebar view={view} setView={navView} projects={projects}/>
        <div style={s.main}>
          <div style={s.topbar}>
            <div style={{fontFamily:"'Syne',sans-serif",fontSize:18,fontWeight:800}}>
              {view==="project"&&currentProject ? (customers.find(c=>c.id===currentProject.customerId)?.name||"Project Detail") : PAGE_TITLES[view]||view}
            </div>
          ...
          <div style={s.content}>
            {view==="dashboard"&&<Dashboard projects={projects} customers={customers} setView={navView} setSelectedProject={setSelectedProject}/>}
            {view==="projects"&&<ProjectsList projects={projects} customers={customers} setView={navView} setSelectedProject={setSelectedProject}/>}
            {view==="project"&&currentProject&&(
              <ProjectDetail
                ...
                setView={navView}
                onEdit={()=>openEdit(currentProject)}
              />
            )}
            {view==="quote_print"&&currentProject&&(
              <div>
                ...
                <QuoteView project={currentProject} customer={customers.find(c=>c.id===currentProject.customerId)}/>
              </div>
            )}
            {view==="pipeline"&&<Pipeline projects={projects} customers={customers} setProjects={setProjects} setView={navView} setSelectedProject={setSelectedProject}/>}
